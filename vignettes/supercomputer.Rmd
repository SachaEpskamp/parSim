...
- Export any external objects (i.e., via the `exports` argument) or `R` packages
  ...
  to run. For the example above, we need to export the `bias` function, so we
  would set `exports = c("bias")`, and no additional packages are needed.
- Decide on the number of `cores` to use for the simulation. The Lisa Compute
  ...
  `parSim` will use ensure at lease one core is available for the main process.
- Finally, we need save the results of each `parSim` function execution to a
  file. Considering the intricacies of supercomputers, it is best to specify the
  file name yourself. To do so, you can pass the file name as a string to the
  `save` argument and ensure that the file name is unique for each `parSim`
  function execution (e.g., by appending the job ID).
...
*Note.* Make sure to adjust the job script to
...
can dispatch an array job using the `sbatch` command. For instance, to submit
the job script above `100` to the Lisa Compute Cluster, we could use the
following command:
...
with a different job ID. The results of each replication will be saved in a
unique file in the `$SIM_DIR/output` directory. In total, each simulation
condition will be replicated `1000 * 100 = 100000` times.

Assuming username `mihaiconstantin`, you can monitor the job queue using the
following command:
...
Finally, you can cancel a job (i.e., or task in an array job) using the
`scancel` command as follows:
...
To run example simulation above on the Lisa Compute Cluster, your
`~/simulation-example` directory should have the following structure:
...
Once the job is completed, you can download the results from the supercomputer
to your local machine using the `scp` command. For instance, to download the
results from the Lisa Compute Cluster, you could use the following command:
...
Finally, you can process the results on your local machine. Suppose you have a
